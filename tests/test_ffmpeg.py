...
class TestFFmpegServiceのconcat_segments:
    """FFmpegServiceのconcat_segmentsメソッドのテスト"""

    def test_セグメントを結合(self, ffmpeg_service, tmp_path):
        """複数のセグメントを結合するテスト"""
        segment_files = [
            tmp_path / "segment_0.mkv",
            tmp_path / "segment_1.mkv",
            tmp_path / "segment_2.mkv"
        ]
        ...
        with patch('builtins.open', m), \
             patch('av1_encoder.ffmpeg.subprocess.run') as mock_run:

            ffmpeg_service.concat_segments(segment_files, audio_file, concat_file, output_file)

            # concat.txtが正しく書き込まれたか確認
            m.assert_called_once_with(concat_file, 'w', encoding='utf-8')
            handle = m()

            # writelines が呼ばれたことを確認
            assert handle.writelines.called

            # ffmpegが正しいコマンドで呼び出されたか確認
            mock_run.assert_called_once()
            called_cmd = mock_run.call_args[0][0]
            assert called_cmd[0] == 'ffmpeg'
            assert '-f' in called_cmd
            assert 'concat' in called_cmd
            assert '-safe' in called_cmd
            assert '0' in called_cmd
            assert '-i' in called_cmd
            assert str(concat_file) in called_cmd
            assert str(audio_file) in called_cmd
            assert '-map' in called_cmd
            assert '0:v:0' in called_cmd
            assert '1:a' in called_cmd
            assert '-c' in called_cmd
            assert 'copy' in called_cmd
            assert str(output_file) in called_cmd

    def test_セグメントを結合_失敗(self, ffmpeg_service, tmp_path):
        """セグメントの結合が失敗した場合のテスト"""
        segment_files = [tmp_path / "segment_0.mkv"]
        audio_file = tmp_path / "audio.aac"
        concat_file = tmp_path / "concat.txt"
        output_file = tmp_path / "output.mp4"

        # subprocess.CalledProcessErrorをモック
        ...
        with patch('builtins.open', m), \
             patch('av1_encoder.ffmpeg.subprocess.run', side_effect=error):

            with pytest.raises(RuntimeError) as exc_info:
                ffmpeg_service.concat_segments(segment_files, audio_file, concat_file, output_file)

            # エラーメッセージが含まれていることを確認
            assert "ビデオ結合失敗" in str(exc_info.value)


class TestFFmpegServiceのconcat_txt_content:
    """FFmpegServiceの_concat_txt_contentメソッドのテスト"""

    def test_concat_txtの内容を生成(self, ffmpeg_service, tmp_path):
        """concat.txtの内容を正しく生成するテスト"""
        segment_files = [
            tmp_path / "segment_0.mkv",
            tmp_path / "segment_1.mkv",
            tmp_path / "segment_2.mkv"
        ...
        content = ffmpeg_service._concat_txt_content(segment_files)
        ...
        for i, line in enumerate(content):
            ...
            assert str(segment_files[i].resolve()) in line

    def test_concat_txtの内容を生成_単一ファイル(self, ffmpeg_service, tmp_path):
        """単一ファイルのconcat.txt内容を生成するテスト"""
        segment_files = [tmp_path / "segment_0.mkv"]

        content = ffmpeg_service._concat_txt_content(segment_files)

        assert len(content) == 1
        assert f"file '{segment_files[0].resolve()}'\n" == content[0]

    def test_concat_txtの内容を生成_空のリスト(self, ffmpeg_service):
        """空のリストでconcat.txt内容を生成するテスト"""
        segment_files = []
